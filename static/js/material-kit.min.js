...
$(document).ready(function () {
  ...
  $('.tagsinput').length != 0 && $('.tagsinput').tagsinput(), $('.bootstrap-tagsinput').addClass('' + a + '-badge'), $('.navbar-color-on-scroll').length != 0 && $(window).on('scroll', materialKit.checkScrollForTransparentNavbar), materialKit.checkScrollForTransparentNavbar(), window_width >= 768 && (big_image = $('.page-header[data-parallax="true"]'), big_image.length != 0 && $(window).on('scroll', materialKit.checkScrollForParallax))
    ...
    !0 != materialKit.misc.colored_shadows || BrowserDetect.browser == 'Explorer' && BrowserDetect.version <= 12 || $('.card:not([data-colored-shadow="false"]) .card-header-image').each(function () {
      if ($card_img = $(this), is_on_dark_screen = $(this).closest('.section-dark, .section-image').length, is_on_dark_screen == 0) {
        var a = $card_img.find('img').attr('src'),
          b = $card_img.closest('.card-rotate').length == 1,
          c = $card_img,
